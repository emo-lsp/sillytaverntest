...
(function () {
  ...
  try {
    legacyLocalStorage_ACU = topLevelWindow_ACU.localStorage;
  } catch (e) {
    legacyLocalStorage_ACU = null;
  }
  ...
  function getTavernOrigin_ACU() {
    try {
      return topLevelWindow_ACU?.location?.origin || window.location.origin;
    } catch (e) {
      ...
      s.textContent = `
              (async () => {
                  try {
                      ...
                      const ext = await import(`${origin}/scripts/extensions.js`);
                      const main = await import(`${origin}/script.js`);
  ...
  async function initTavernSettingsBridge_ACU() {
    ...
    // 1) 取 saveSettings()
    try {
      const origin = getTavernOrigin_ACU();
      const mod = await import(`${origin}/script.js`);
      if (mod) {
  ...
  function persistTavernSettings_ACU() {
    try {
      ...
      // 兜底：优先 debounced
      if (typeof topLevelWindow_ACU.saveSettingsDebounced === 'function') {
        topLevelWindow_ACU.saveSettingsDebounced();
        return;
      }
  ...
  function migrateKeyToTavernStorageIfNeeded_ACU(key) {
    ...
    if (legacy !== null && typeof legacy !== 'undefined') {
      ...
      try {
        legacyLocalStorage_ACU.removeItem(key);
      } catch (e) {
        /* ignore */
      }
      return true;
  ...
  function ensurePlotPromptsArray_ACU(plotSettings) {
    ...
    plotSettings.prompts = [
      ...
      },
      {
        ...
        name: '最终注入指令 (Storyteller Directive)',
        role: 'system',
        content: legacy.finalSystemDirective || '',
        deletable: false,
      },
    ];
  }
...
-->
  ...
  function ensureProfileExists_ACU(code, { seedFromCurrent = true } = {}) {
    ...
    if (!hasSettings) {
      ...
      try {
        writeProfileSettingsToStorage_ACU(c, seed);
      } catch (e) {
        logWarn_ACU('[Profile] seed settings failed:', e);
      }
    }
    if (!hasTemplate) {
      const seedTemplate = seedFromCurrent
        ? TABLE_TEMPLATE_ACU || DEFAULT_TABLE_TEMPLATE_ACU
        : DEFAULT_TABLE_TEMPLATE_ACU;
      try {
        writeProfileTemplateToStorage_ACU(c, seedTemplate);
      } catch (e) {
        logWarn_ACU('[Profile] seed template failed:', e);
      }
    }
  }
  ...
  function renderPromptSegments_ACU(segments) {
    ...
    segments.forEach((segment, index) => {
      ...
      const mainSlot =
        (segment &&
          (String(segment.mainSlot || '').toUpperCase() || (segment.isMain ? 'A' : segment.isMain2 ? 'B' : ''))) ||
        '';
      ...
      const segmentHtml = `
              <div class="prompt-segment" id="${segmentId}" data-main-slot="${escapeHtml_ACU(mainSlot)}" ${
        isMainPrompt ? `style="border-left: 3px solid ${borderColor};"` : ''
                  ...
                  <div class="prompt-segment-toolbar">
                      <div style="display:flex; align-items:center; gap:8px;">
                          <select class="prompt-segment-role">
                              <option value="assistant" ${
                                roleUpper === 'AI' || roleUpper === 'ASSISTANT' || roleLower === 'assistant'
                                  ? 'selected'
                                  : ''
                              }>AI</option>
                              <option value="SYSTEM" ${
                                roleUpper === 'SYSTEM' || roleLower === 'system' ? 'selected' : ''
                              }>系统</option>
                              <option value="USER" ${
                                roleUpper === 'USER' || roleLower === 'user' ? 'selected' : ''
                              }>用户</option>
                          </select>
                      ...
                      <button class="prompt-segment-delete-btn" data-index="${index}" style="${
        isMainPrompt ? 'display:none;' : ''
      }">-</button>
                  </div>
  ...
  topLevelWindow_ACU.AutoCardUpdaterAPI = {
          ...
          if (chat && chat.length > 0) {
            ...
            if (targetMessage) {
              ...
              if (Object.keys(summaryData).some(k => k.startsWith('sheet_'))) {
                targetMessage.TavernDB_ACU_SummaryData = sanitizeChatSheetsObject_ACU(summaryData, {
                  ensureMate: true,
                });
    ...
    importTemplate: async function () {
      try {
        return await importTableTemplate_ACU();
      } catch (e) {
    ...
    exportTemplate: async function () {
      try {
        return await exportTableTemplate_ACU();
      } catch (e) {
        logError_ACU('exportTemplate failed:', e);
        return false;
      }
    },
    resetTemplate: async function () {
      try {
        return await resetTableTemplate_ACU();
      } catch (e) {
        logError_ACU('resetTemplate failed:', e);
        return false;
      }
    },
    resetAllDefaults: async function () {
      try {
    ...
    importCombinedSettings: async function () {
      try {
    ...
    exportCombinedSettings: async function () {
      try {
    ...
    overrideWithTemplate: async function () {
      try {
    ...
    importTxtAndSplit: async function () {
      try {
    ...
    injectImportedSelected: async function () {
      try {
    ...
    injectImportedStandard: async function () {
      try {
        return await handleInjectSplitEntriesStandard_ACU();
      } catch (e) {
        logError_ACU('injectImportedStandard failed:', e);
        return false;
      }
    },
    injectImportedSummary: async function () {
      try {
        return await handleInjectSplitEntriesSummary_ACU();
      } catch (e) {
        logError_ACU('injectImportedSummary failed:', e);
        return false;
      }
    },
    injectImportedFull: async function () {
      try {
        return await handleInjectSplitEntriesFull_ACU();
      } catch (e) {
        logError_ACU('injectImportedFull failed:', e);
        return false;
      }
    },
    deleteImportedEntries: async function () {
      try {
        return await deleteImportedEntries_ACU();
      } catch (e) {
        logError_ACU('deleteImportedEntries failed:', e);
        return false;
      }
    },
    clearImportedEntries: async function (clearAll = true) {
      try {
        return await clearImportedEntries_ACU(!!clearAll);
      } catch (e) {
        logError_ACU('clearImportedEntries failed:', e);
        return false;
      }
    },
    clearImportCache: async function (clearAll = true) {
      try {
        return await clearImportLocalStorage_ACU(!!clearAll);
      } catch (e) {
    ...
    // 合并总结
    mergeSummaryNow: async function () {
      try {
  ...
  function _acuNormalizeToastArgs_ACU(type, message, titleOrOptions = {}, maybeOptions = {}) {
    ...
    const defaultTimeOut =
      type === 'success' ? 2500 : type === 'info' ? 2500 : type === 'warning' ? 3500 : type === 'error' ? 5000 : 2500;

    const isNarrow = (() => {
      try {
        const w =
          topLevelWindow_ACU && typeof topLevelWindow_ACU.innerWidth === 'number'
  ...
  function showToastr_ACU(type, message, titleOrOptions = {}, maybeOptions = {}) {
    ...
    try {
      const key = `${type}|${title}|${String(message)
        .replace(/<[^>]*>/g, '')
        .slice(0, 120)}`;
      const now = Date.now();
    ...
    if (isObject(target) && isObject(source)) {
      Object.keys(source).forEach(key => {
        if (isObject(source[key])) {
          ...
          else output[key] = deepMerge_ACU(target[key], source[key]);
        } else {
  ...
  function ensureSheetOrderNumbers_ACU(dataObj, { baseOrderKeys = null, forceRebuild = false } = {}) {
    if (!dataObj || typeof dataObj !== 'object') return false;
    const sheetKeys =
      Array.isArray(baseOrderKeys) && baseOrderKeys.length
  ...
  function getTemplateSheetKeys_ACU() {
    ...
    return keys.sort((a, b) => {
      const ao = Number.isFinite(templateObj[a]?.[TABLE_ORDER_FIELD_ACU])
        ? templateObj[a][TABLE_ORDER_FIELD_ACU]
        : Infinity;
      const bo = Number.isFinite(templateObj[b]?.[TABLE_ORDER_FIELD_ACU])
        ? templateObj[b][TABLE_ORDER_FIELD_ACU]
        : Infinity;
        ...
        if (message.TavernDB_ACU_Data) {
          ...
          Object.keys(standardData).forEach(k => {
            if (
              ...
              !foundSheets[k] &&
              standardData[k].name &&
              !isSummaryOrOutlineTable_ACU(standardData[k].name)
            ) {
        ...
        if (message.TavernDB_ACU_SummaryData) {
          ...
          Object.keys(summaryData).forEach(k => {
            if (
              ...
              !foundSheets[k] &&
              summaryData[k].name &&
              isSummaryOrOutlineTable_ACU(summaryData[k].name)
            ) {
  ...
  async function seedGreetingLocalDataFromTemplate_ACU() {
    try {
      ...
      // 更新内存（但不触发世界书注入）
      currentJsonTableData_ACU = reorderDataBySheetKeys_ACU(
        JSON.parse(JSON.stringify(baseData)),
        getSortedSheetKeys_ACU(baseData),
      );
      return true;
    } catch (e) {
  ...
  function saveSettings_ACU() {
    try {
      ...
      if (store && store._isTavern) {
        ...
        console.warn(
          `[${SCRIPT_ID_PREFIX_ACU}] 未连接到可持久化的 extension_settings：本次保存仅在内存中生效，刷新会丢失。请检查顶层 bridge 是否注入成功。`,
        );
        // showToastr_ACU 可能尚未初始化，故 try/catch
        try {
          showToastr_ACU('warning', '⚠️ 当前未连接到酒馆服务端设置，本次修改刷新后会丢失。请打开控制台查看原因。', {
            timeOut: 8000,
          });
        } catch (e) {}
        // 异步再尝试一次初始化（不阻塞 UI）
  ...
  async function onLoopGenerationEnded_ACU() {
    ...
    if (planningGuard_ACU.ignoreNextGenerationEndedCount > 0) {
      ...
      logDebug_ACU(
        `[剧情推进] [Loop] Ignoring planning-triggered GENERATION_ENDED (${planningGuard_ACU.ignoreNextGenerationEndedCount} left).`,
      );
      return;
    }
  ...
  async function savePlotToLatestMessage_ACU() {
    ...
    logDebug_ACU(
      '[剧情推进] [Plot] planningGuard_ACU.ignoreNextGenerationEndedCount:',
      planningGuard_ACU.ignoreNextGenerationEndedCount,
    );
    logDebug_ACU(
      '[剧情推进] [Plot] tempPlotToSave_ACU:',
      tempPlotToSave_ACU ? `长度=${tempPlotToSave_ACU.length}` : '(空)',
    );
    ...
    if (planningGuard_ACU.ignoreNextGenerationEndedCount > 0) {
      planningGuard_ACU.ignoreNextGenerationEndedCount--;
      logDebug_ACU(
        `[剧情推进] [Plot] Ignoring planning-triggered GENERATION_ENDED (${planningGuard_ACU.ignoreNextGenerationEndedCount} left).`,
      );
      return;
    }
    ...
    if (tempPlotToSave_ACU) {
      ...
      if (chat && chat.length > 0) {
        ...
        logDebug_ACU(
          '[剧情推进] [Plot] 最后一条消息:',
          lastMessage ? `is_user=${lastMessage.is_user}, name=${lastMessage.name}` : '(空)',
        );
  ...
  async function getWorldbookContentForPlot_ACU(apiSettings, userMessage, extraBaseText = '') {
    ...
    try {
      ...
      const recursionAllowedConstantText = recursionAllowedConstants
        .map(e => e.content || '')
        .join('\n')
        .toLowerCase();
      const initialScanText = [historyAndUserText, recursionAllowedConstantText, extraBaseLower]
        .filter(Boolean)
        .join('\n');
      ...
      if (recursionDepth >= MAX_RECURSION_DEPTH) {
        logWarn_ACU('[剧情推进] Worldbook recursion reached max depth of ' + MAX_RECURSION_DEPTH + '. Breaking loop.');
      }
      ...
      logDebug_ACU(
        '[剧情推进] Combined worldbook content generated, length: ' +
  ...
  function loadSettings_ACU() {
    ...
    if (
      !globalMeta_ACU.migratedLegacySingleStore &&
      (legacySettingsObj || store?.getItem?.(STORAGE_KEY_CUSTOM_TEMPLATE_ACU))
    ) {
    ...
    try {
      ...
      if (savedSettings) {
        ...
        if (!settings_ACU.plotSettings.plotWorldbookConfig) {
          ...
          const legacyBooks = Array.isArray(settings_ACU.plotSettings.selectedWorldbooks)
            ? settings_ACU.plotSettings.selectedWorldbooks
            : [];
    ...
    if ($popupInstance_ACU) {
      ...
      const $tableContextExtractTagsInput = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-table-context-extract-tags`,
      );
      if ($tableContextExtractTagsInput.length)
        $tableContextExtractTagsInput.val(settings_ACU.tableContextExtractTags || '');
      const $tableContextExcludeTagsInput = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-table-context-exclude-tags`,
      );
      if ($tableContextExcludeTagsInput.length)
        $tableContextExcludeTagsInput.val(settings_ACU.tableContextExcludeTags || '');
      ...
      if ($popupInstance_ACU) {
        $popupInstance_ACU
          .find(`input[name="${SCRIPT_ID_PREFIX_ACU}-api-mode"][value="${settings_ACU.apiMode}"]`)
          .prop('checked', true);
    ...
    try {
      ...
      tavernProfiles.forEach(profile => {
        if (profile.api && profile.preset) {
          // Ensure it's a valid API profile
          const option = $('<option>', {
            value: profile.id,
  ...
  function saveCustomCharCardPrompt_ACU() {
    ...
    if (
      !newPromptSegments ||
      newPromptSegments.length === 0 ||
      (newPromptSegments.length === 1 && !newPromptSegments[0].content.trim())
    ) {
      ...
      reader.onload = readerEvent => {
        ...
        try {
          ...
          if (
            ...
            jsonData.some(item => typeof item.role === 'undefined' || typeof item.content === 'undefined')
          ) {
  ...
  function saveAutoUpdateThreshold_ACU({ silent = false, skipReload = false } = {}) {
    ...
    if (!isNaN(newT) && newT >= 0) {
      ...
      if (!silent)
        showToastr_ACU(
          'warning',
          `阈值 "${valStr}" 无效。请输入一个大于等于0的整数。恢复为: ${settings_ACU.autoUpdateThreshold}`,
        );
  ...
  function saveAutoUpdateTokenThreshold_ACU({ silent = false, skipReload = false } = {}) {
    ...
    if (!isNaN(newT) && newT >= 0) {
      ...
      if (!silent)
        showToastr_ACU(
          'warning',
          `Token阈值 "${valStr}" 无效。请输入一个大于等于0的整数。恢复为: ${settings_ACU.autoUpdateTokenThreshold}`,
        );
      $autoUpdateTokenThresholdInput_ACU.val(settings_ACU.autoUpdateTokenThreshold);
    }
  }
  ...
  function saveAutoUpdateFrequency_ACU({ silent = false, skipReload = false } = {}) {
    ...
    if (!isNaN(newF) && newF >= 1) {
      ...
      if (!silent)
        showToastr_ACU(
          'warning',
          `更新频率 "${valStr}" 无效。请输入一个大于0的整数。恢复为: ${settings_ACU.autoUpdateFrequency}`,
        );
  ...
  function saveUpdateBatchSize_ACU({ silent = false, skipReload = false } = {}) {
    ...
    if (!isNaN(newBatchSize) && newBatchSize >= 1) {
      ...
      if (!silent)
        showToastr_ACU(
          'warning',
          `批处理大小 "${valStr}" 无效。请输入一个大于0的整数。恢复为: ${settings_ACU.updateBatchSize}`,
        );
  ...
  function saveSkipUpdateFloors_ACU({ silent = false, skipReload = false } = {}) {
    ...
    if (!isNaN(newSkip) && newSkip >= 0) {
      ...
      if (!silent)
        showToastr_ACU(
          'warning',
          `跳过更新楼层 "${valStr}" 无效。请输入一个大于等于0的整数。恢复为: ${settings_ACU.skipUpdateFloors || 0}`,
        );
  ...
  function saveImportSplitSize_ACU() {
    ...
    if (!isNaN(newSize) && newSize >= 100) {
      ...
      showToastr_ACU(
        'warning',
        `导入分割大小 "${valStr}" 无效。请输入一个大于等于100的整数。恢复为: ${settings_ACU.importSplitSize}`,
      );
  ...
  async function triggerAutomaticUpdateIfNeeded_ACU() {
    ...
    if (totalAiMessages > lastTotalAiMessages_ACU) {
      logDebug_ACU(
        `ACU: AI Message count increased (${lastTotalAiMessages_ACU} -> ${totalAiMessages}). Waiting ${AUTO_UPDATE_FLOOR_INCREASE_DELAY_ACU}ms...`,
      );
    ...
    // 预计算所有 AI 消息索引
    const allAiMessageIndices = liveChat.map((msg, index) => (!msg.is_user ? index : -1)).filter(index => index !== -1);
    ...
    if (isDatabaseEmpty && allAiMessageIndices.length > 0) {
      logDebug_ACU(
        'ACU Auto-Trigger: Database is empty (First Floor scenario). Will use normal frequency-based update logic.',
      );
      // [优化] 不再强制触发所有表格的更新
    ...
    // [优化] 统一使用频率逻辑，无论是否是首次初始化
    {
      ...
      for (const sheetKey of sheetKeys) {
        ...
        const threshold =
          (tableConfig.contextDepth || 0) > 0 ? tableConfig.contextDepth : settings_ACU.autoUpdateThreshold || 3;
        ...
        logDebug_ACU(
          `[Trigger Check] Table: ${table.name}, TotalAI: ${totalAiMessages}, Skip: ${skipFloors}, LastUpdated: ${lastUpdatedAiFloor}, Unrecorded: ${effectiveUnrecordedFloors}, Freq: ${frequency}`,
        );
        ...
        if (effectiveUnrecordedFloors >= frequency && threshold > 0) {
          ...
          // 1. 计算有效范围的截止点（跳过楼层处理）
          const effectiveAiIndices = skipFloors > 0 ? allAiMessageIndices.slice(0, -skipFloors) : allAiMessageIndices;
          ...
          logDebug_ACU(
            `[Trigger Check] EffIndicesLen: ${effectiveAiIndices.length}, StartIndex: ${startIndexInAiArray}`,
          );
          ...
          if (startIndexInAiArray < effectiveAiIndices.length) {
            ...
            logDebug_ACU(
              `[Trigger Check] Unupdated: ${unupdatedAiIndices.length}, ContextScope: ${contextScopeIndices.length}`,
            );
    ...
    if (groupKeys.length > 0) {
      ...
      for (const key of groupKeys) {
        ...
        logDebug_ACU(
          `Group update for ${group.sheetNames.join(', ')} completed. Forcing data refresh for next group...`,
        );
  ...
  function renderManualTableSelector_ACU() {
    ...
    if (
      !Array.isArray(settings_ACU.manualSelectedTables) ||
      JSON.stringify(settings_ACU.manualSelectedTables) !== JSON.stringify(resolvedSelection)
    ) {
      settings_ACU.manualSelectedTables = resolvedSelection;
      saveSettings_ACU();
    }
    let html =
      '<div class="acu-table-selector" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:8px;max-height:240px;overflow:auto;padding:8px;border:1px solid var(--border-normal);border-radius:8px;background:var(--bg-secondary);">';
  ...
  function renderImportTableSelector_ACU() {
    ...
    const selectedSet = new Set(resolvedSelection);
    if (
      !Array.isArray(settings_ACU.importSelectedTables) ||
      JSON.stringify(settings_ACU.importSelectedTables) !== JSON.stringify(resolvedSelection)
    ) {
      ...
      try {
        ...
        if (charLorebook) {
          ...
          if (charLorebook !== worldbookConfig.injectionTarget) {
            logDebug_ACU(
              `Cleaning up bound worldbook "${charLorebook}" as target is "${worldbookConfig.injectionTarget}"`,
            );
  ...
  async function resetScriptStateForNewChat_ACU(chatFileName) {
    ...
    if (
      ...
      chatFileName.trim() === '' ||
      chatFileName.trim() === 'null'
    ) {
      logWarn_ACU(
        `ACU: Received invalid chat file name: "${chatFileName}". This can happen after an update error. Ignoring event to preserve current state.`,
      );
      // 保持当前状态不变，防止数据库被意外清除
      return;
    }
    ...
    try {
      ...
      if (uidsToDelete.length > 0) {
        ...
        if (settings_ACU.knownCustomEntryNames) {
          if (settings_ACU.dataIsolationEnabled) {
            settings_ACU.knownCustomEntryNames = settings_ACU.knownCustomEntryNames.filter(
              n => !n.startsWith(isolationPrefix),
            );
          } else {
  ...
  async function purgeSheetKeysFromChatHistoryHard_ACU(sheetKeysToPurge) {
    ...
    for (let i = 0; i < chat.length; i++) {
      ...
      if (Array.isArray(msg.TavernDB_ACU_ModifiedKeys)) {
        ...
        if (any) {
          msg.TavernDB_ACU_ModifiedKeys = next;
          msgChanged = true;
        }
      }
    ...
    try {
      ...
      if (existingEntry) {
        ...
        if (needsUpdate) {
          ...
          logDebug_ACU(
            `Successfully updated the outline table lorebook entry. enabled=${outlineEntryEnabled} (0TK占用模式=${zeroTkOccupyMode})`,
          );
        } else {
        ...
        logDebug_ACU(
          `Outline table lorebook entry not found. Created a new one. enabled=${outlineEntryEnabled} (0TK占用模式=${zeroTkOccupyMode})`,
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (!isImport) {
        const uidsToDelete = allEntries
          .filter(
            e =>
              e.comment && (e.comment.startsWith(SUMMARY_ENTRY_PREFIX) || e.comment.startsWith(SMALL_SUMMARY_PREFIX)),
          )
          .map(e => e.uid);
  ...
  async function updateReadableLorebookEntry_ACU(createIfNeeded = false, isImport = false) {
    ...
    if (primaryLorebookName) {
      try {
        ...
        // [修改] 加入隔离标识前缀
        const isoPrefix = getIsolationPrefix_ACU();
        const baseReadableComment = isImport
          ? `${IMPORT_PREFIX}TavernDB-ACU-ReadableDataTable`
          : 'TavernDB-ACU-ReadableDataTable';
        ...
        const WRAPPER_START_COMMENT =
          isoPrefix + (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-WrapperStart` : 'TavernDB-ACU-WrapperStart');
        const WRAPPER_END_COMMENT =
          isoPrefix + (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-WrapperEnd` : 'TavernDB-ACU-WrapperEnd');
        ...
        if (isDatabaseEmpty) {
          ...
          const memoryStartOld = entries.find(
            e =>
              e.comment ===
              isoPrefix + (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-MemoryStart` : 'TavernDB-ACU-MemoryStart'),
          );
          const memoryEndOld = entries.find(
            e =>
              e.comment ===
              isoPrefix + (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-MemoryEnd` : 'TavernDB-ACU-MemoryEnd'),
          );
          ...
          if (toDelete.length > 0) {
            ...
            logDebug_ACU(
              `Deleted ${toDelete.length} lorebook entries because database is empty/reset (readable + wrappers).`,
            );
          }
    ...
    try {
      ...
      sortedTableKeys.forEach(sheetKey => {
        ...
        if (config.splitByRow) {
          ...
          const baseOrder = allocConsecutiveOrderBlock_ACU(
            usedOrders,
            Math.max(1, blockSpan),
            nextCustomExportOrder,
            1,
            99999,
          );
      ...
      Object.keys(mergedEntriesMap).forEach(key => {
        ...
        const blockEntries = [];
        // 准备所有合并表格的表头内容
        const allHeadersContent = group.tableHeaders
          ? group.tableHeaders
              .map(th => {
                return `# ${th.name}\n\n| ${th.headers.join(' | ')} |\n|${th.headers.map(() => '---').join('|')}|`;
              })
  ...
  async function updateImportantPersonsRelatedEntries_ACU(importantPersonsTable, isImport = false) {
    ...
    const basePersonIndexComment = isImport
      ? `${IMPORT_PREFIX}TavernDB-ACU-ImportantPersonsIndex`
      : 'TavernDB-ACU-ImportantPersonsIndex';
    ...
    try {
      ...
      if (!isImport) {
        ...
        const uidsToDelete = allEntries
          .filter(
            e =>
              e.comment &&
              (e.comment.startsWith(PERSON_ENTRY_PREFIX) ||
                e.comment === PERSON_INDEX_COMMENT ||
                e.comment.includes('PersonsHeader')),
          )
          .map(e => e.uid);
      ...
      const personsHeaderEntryData = {
        ...
        keys: [
          isoPrefix + (isImport ? `${IMPORT_PREFIX}TavernDB-ACU-PersonsHeader-Key` : 'TavernDB-ACU-PersonsHeader-Key'),
        ],
        enabled: true,
        type: 'constant',
        order: null,
      ...
      if (allCreates.length > 0) {
        ...
        try {
          ...
          rows.forEach(e => {
            if (e?.uid) updates.push({ uid: e.uid, order: personsOrderBlockBase + 1 });
          });
  ...
  async function saveIndependentTableToChatHistory_ACU(
    ...
    // }
    let isolatedData = targetMessage.TavernDB_ACU_IsolatedData
      ? JSON.parse(JSON.stringify(targetMessage.TavernDB_ACU_IsolatedData))
      : {};
    ...
    if (actuallyModifiedKeys.length > 0) {
      ...
      logDebug_ACU(
        `[Tracking] Recorded modified keys for tag [${
          currentIsolationKey || '无标签'
        }] at index ${finalIndex}: ${currentTagData.modifiedKeys.join(', ')}`,
      );
    }
    ...
    if (updateGroupKeys && updateGroupKeys.length > 0 && actuallyModifiedKeys.length > 0) {
      ...
      logDebug_ACU(
        `[Merge Update Success] Group keys for tag [${
          currentIsolationKey || '无标签'
        }] recorded at index ${finalIndex}: ${currentTagData.updateGroupKeys.join(', ')}`,
      );
    } else if (updateGroupKeys && updateGroupKeys.length > 0 && actuallyModifiedKeys.length === 0) {
      logDebug_ACU(
        `[Merge Update Failed] No tables were modified for tag [${
          currentIsolationKey || '无标签'
        }]. Group keys NOT recorded: ${updateGroupKeys.join(', ')}`,
      );
    }
    ...
    logDebug_ACU(
      `Saved ${keysToSave.length} tables for tag [${
        currentIsolationKey || '无标签'
      }] to message at index ${finalIndex}. Actually modified: ${actuallyModifiedKeys.length} tables.`,
    );
      ...
      if (
          ...
          if (!window.original_TavernHelper_generate_ACU) {
            if (window.TavernHelper && typeof window.TavernHelper.generate === 'function') {
              ...
              window.TavernHelper.generate = async function (...args) {
                ...
                if (
                  !settings_ACU.plotSettings.enabled ||
                  isProcessing_Plot_ACU ||
                  loopState_ACU.isRetrying ||
                  options.should_stream
                ) {
                  ...
                  if (userMessage) {
                    ...
                    try {
                      ...
                      if (
                        ...
                        const loopSettings =
                          settings_ACU.plotSettings.loopSettings || DEFAULT_PLOT_SETTINGS_ACU.loopSettings;
                        loopState_ACU.awaitingReply = false;
                        await enterLoopRetryFlow_ACU({ loopSettings, shouldDeleteAiReply: false });
                        return;
                      }
        ...
        if (SillyTavern_API_ACU.eventTypes.GENERATION_AFTER_COMMANDS) {
          SillyTavern_API_ACU.eventSource.on(
            SillyTavern_API_ACU.eventTypes.GENERATION_AFTER_COMMANDS,
            async (type, params, dryRun) => {
              ...
              try {
                const lastMsgText =
                  ...
                  SillyTavern_API_ACU.chat[SillyTavern_API_ACU.chat.length - 1]?.is_user
                ...
                if (shouldSkipPlotIntercept_ACU(lastMsgText) || shouldSkipPlotIntercept_ACU(boxText)) {
                  logDebug_ACU(
                    '[剧情推进] Skip GENERATION_AFTER_COMMANDS due to recent TavernHelper.generate interception.',
                  );
                  return;
                }
              } catch (e) {}
                      ...
                      if (finalMessage.manual) {
                        ...
                        try {
                          ...
                          if (
                            lastNow &&
                            lastNow.is_user &&
                            String(lastNow.mes || '') === String(messageToProcess || '')
                          ) {
                            if (typeof SillyTavern_API_ACU.deleteLastMessage === 'function') {
                              await SillyTavern_API_ACU.deleteLastMessage();
                            } else if (window.SillyTavern?.deleteLastMessage) {
  ...
  function getImportStablePrefix_ACU() {
    return '外部导入-';
  }
  // 当前按用户要求：外部导入不自动清理，因此无需批次隔离；统一使用稳定前缀即可
  function getImportBatchPrefix_ACU() {
    return getImportStablePrefix_ACU();
  }
  ...
  async function deleteImportedEntries_ACU() {
    ...
    try {
      ...
      if (uidsToDelete.length > 0) {
        await TavernHelper_API_ACU.deleteLorebookEntries(targetLorebook, uidsToDelete);
        logDebug_ACU(
          `Successfully deleted ${uidsToDelete.length} imported entries from ${targetLorebook} (Isolation: ${settings_ACU.dataIsolationEnabled}).`,
        );
    ...
    if (savedEntriesJson) {
      try {
        ...
        if (Array.isArray(chunks) && chunks.length > 0) {
          ...
          if (savedStatusJson) {
            try {
              status = JSON.parse(savedStatusJson);
            } catch (e) {
              status = null;
            }
          }
  ...
  async function processImportedTxtAsUpdates_ACU() {
    ...
    if (savedStatusJson) {
      try {
        ...
        if (
          savedStatus.total === allChunks.length &&
          (typeof savedStatus.selectionSig === 'undefined' || savedStatus.selectionSig === selectionSig)
        ) {
    ...
    for (let i = status.currentIndex; i < allChunks.length; i++) {
      ...
      while (attempt < MAX_RETRIES && !success) {
        ...
        success = await proceedWithCardUpdate_ACU(
          [mockMessage],
          toastMessage,
          -1,
          true,
    ...
    try {
      ...
      if (existingEntry) {
        await TavernHelper_API_ACU.setLorebookEntries(importTargetLorebook, [
          { ...newEntryData, uid: existingEntry.uid },
        ]);
  ...
  async function populatePlotWorldbookList_ACU() {
    ...
    try {
      ...
      books.forEach(book => {
        ...
        const itemHtml = `
                  <div class="qrf_worldbook_list_item ${isSelected ? 'selected' : ''}" data-book-name="${escapeHtml_ACU(
          book.name,
        )}">
    ...
    try {
      ...
      for (const bookName of bookNames) {
        ...
        if (bookData && bookData.entries) {
          ...
          html += `<div class="qrf_worldbook_entry_header" data-book-name="${escapeHtml_ACU(
            bookName,
          )}" style="margin-bottom: 5px; font-weight: bold; border-bottom: 1px solid;">${escapeHtml_ACU(
            bookName,
          )}</div>`;
          bookData.entries.forEach(entry => {
            ...
            html += `
                          <div class="qrf_worldbook_entry_item">
                              <input type="checkbox" id="plot-wb-entry-${entry.uid}" data-book="${escapeHtml_ACU(
              bookName,
            )}" data-uid="${entry.uid}" ${isChecked ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
                              <label for="plot-wb-entry-${entry.uid}" ${
              isDisabled ? 'style="opacity:0.6; text-decoration: line-through;"' : ''
            }>${escapeHtml_ACU(entry.comment || `条目 ${entry.uid}`)}</label>
                          </div>`;
          });
        }
      }
      ...
      for (const bookName of bookNames) {
        ...
        if (bookData && bookData.entries) {
          ...
          if (typeof worldbookConfig.enabledEntries[bookName] === 'undefined') {
            ...
            worldbookConfig.enabledEntries[bookName] = bookData.entries
              .filter(entry => {
                ...
                // 过滤自动生成的条目
                if (
                  comment.startsWith('TavernDB-ACU-') ||
          ...
          bookData.entries.forEach(entry => {
            ...
            html += `
                          <div class="qrf_worldbook_entry_item">
                              <input type="checkbox" id="wb-entry-${entry.uid}" data-book="${escapeHtml_ACU(
              bookName,
            )}" data-uid="${entry.uid}" ${isChecked ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
                              <label for="wb-entry-${entry.uid}" ${
              isDisabled ? 'style="opacity:0.6; text-decoration: line-through;"' : ''
            }>${escapeHtml_ACU(entry.comment || `条目 ${entry.uid}`)}</label>
                          </div>`;
          });
  ...
  async function openAutoCardPopup_ACU() {
    ...
    setTimeout(async () => {
      ...
      $charCardPromptSegmentsContainer_ACU = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-prompt-segments-container`,
      );
      ...
      const $loadCharCardPromptFromJsonButton_ACU = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-load-char-card-prompt-from-json`,
      );
      ...
      $autoUpdateTokenThresholdInput_ACU = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-auto-update-token-threshold`,
      );
      $saveAutoUpdateTokenThresholdButton_ACU = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-save-auto-update-token-threshold`,
      );
      $autoUpdateFrequencyInput_ACU = $popupInstance_ACU.find(`#${SCRIPT_ID_PREFIX_ACU}-auto-update-frequency`);
      $saveAutoUpdateFrequencyButton_ACU = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-save-auto-update-frequency`,
      );
      ...
      const $importCombinedSettingsButton = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-import-combined-settings`,
      );
      const $exportCombinedSettingsButton = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-export-combined-settings`,
      );
      ...
      const $refreshTavernProfilesButton = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-refresh-tavern-api-profiles`,
      );
      const $worldbookSourceRadios = $popupInstance_ACU.find(`input[name="${SCRIPT_ID_PREFIX_ACU}-worldbook-source"]`);
      ...
      const $dataIsolationDeleteButton = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-data-isolation-delete-entries`,
      ); // [新增]
      const $dataIsolationCombo = $popupInstance_ACU.find(`#${SCRIPT_ID_PREFIX_ACU}-data-isolation-combo`);
      const $dataIsolationHistoryToggle = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-data-isolation-history-toggle`,
      );
      ...
      // [新增] 外部导入世界书选择器的事件绑定
      const $refreshImportWorldbooksButton = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-refresh-import-worldbooks`,
      );
      if ($refreshImportWorldbooksButton.length) {
        $refreshImportWorldbooksButton.on('click', populateImportWorldbookTargetSelector_ACU);
      }
      const $importWorldbookTargetSelect = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-import-worldbook-injection-target`,
      );
      const $importWorldbookTargetFilter = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-import-worldbook-injection-target-filter`,
      );
      ...
      if ($useMainApiCheckbox_ACU.length) {
        $useMainApiCheckbox_ACU.on('change', function () {
          ...
          showToastr_ACU(
            'info',
            `自定义API已切换为 ${settings_ACU.apiConfig.useMainApi ? '使用主API' : '使用独立配置'}`,
          );
        });
      }
      ...
      if ($loadCharCardPromptFromJsonButton_ACU.length)
        $loadCharCardPromptFromJsonButton_ACU.on('click', loadCharCardPromptFromJson_ACU);
      ...
      const bindAutoSaveNumberInput_ACU = ($input, saveFn, debounceMs = 450) => {
        ...
        $input
          .off('input.acu_autosave change.acu_autosave blur.acu_autosave')
      ...
      // [新增] 删除本地数据按钮绑定
      const $deleteCurrentLocalDataButton = $popupInstance_ACU.find(
        `#${SCRIPT_ID_PREFIX_ACU}-delete-current-local-data`,
      );
      ...
      if ($deleteCurrentLocalDataButton.length) {
        $deleteCurrentLocalDataButton.on('click', function () {
          ...
          const identityText = settings_ACU.dataIsolationEnabled
            ? `标识 [${settings_ACU.dataIsolationCode}]`
            : '所有标识';
          const rangeText =
            startFloor && endFloor
              ? `第${startFloor}到${endFloor}AI楼层`
              : startFloor
              ? `从第${startFloor}AI楼层开始`
              : endFloor
              ? `到第${endFloor}AI楼层结束`
              : '全部AI楼层';
          ...
          if (
            confirm(
              `警告：这将永久删除当前聊天记录中${rangeText}所有属于 ${identityText} 的数据库数据。\n\n此操作不可恢复！\n\n确定要继续吗？`,
            )
          ) {
      ...
      if ($deleteAllLocalDataButton.length) {
        $deleteAllLocalDataButton.on('click', function () {
          ...
          if (
            confirm(
              `严重警告：这将永久删除当前聊天记录中${rangeText}【所有】数据库数据，无论其标识是什么。\n\n此操作不可恢复！\n\n确定要继续吗？`,
            )
          ) {
            // 二次确认
...
insertRow(1, ["时间2", "大纲事件2...", "关键词"]);
      ...
      if ($startLoopBtn.length) {
        $startLoopBtn.on('click', function () {
          const duration = parseInt(
            $popupInstance_ACU.find(`#${SCRIPT_ID_PREFIX_ACU}-plot-loop-total-duration`).val(),
            10,
          );
        ...
        const setPlotEntriesSelection_ACU = async mode => {
          ...
          for (const bookName of bookNames) {
            ...
            if (bookData?.entries?.length) {
              ...
              try {
                entries = await TavernHelper_API_ACU.getLorebookEntries(bookName);
              } catch (e) {
                entries = [];
              }
            }
    ...
    function loadPlotSettingsToUI_ACU() {
      ...
      // 提示词
      $popupInstance_ACU
        .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-main-prompt`)
        .val(getPlotPromptContentById_ACU('mainPrompt'));
      $popupInstance_ACU
        .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-system-prompt`)
        .val(getPlotPromptContentById_ACU('systemPrompt'));
      $popupInstance_ACU
        .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-final-directive`)
        .val(getPlotPromptContentById_ACU('finalSystemDirective'));
      ...
      $popupInstance_ACU
        .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-quick-reply-content`)
        .val(loopSettings.quickReplyContent || '');
      ...
      $popupInstance_ACU
        .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-context-extract-tags`)
        .val(plotSettings.contextExtractTags || '');
      $popupInstance_ACU
        .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-context-exclude-tags`)
        .val(plotSettings.contextExcludeTags || '');
    ...
    function loadPlotPresetToUI_ACU(preset) {
      ...
      if (preset.loopSettings) {
        $popupInstance_ACU
          .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-quick-reply-content`)
          .val(preset.loopSettings.quickReplyContent || '');
        ...
        $popupInstance_ACU
          .find(`#${SCRIPT_ID_PREFIX_ACU}-plot-loop-total-duration`)
          .val(preset.loopSettings.loopTotalDuration ?? 0);
      ...
      if (preset.loopSettings)
        settings_ACU.plotSettings.loopSettings = { ...settings_ACU.plotSettings.loopSettings, ...preset.loopSettings };
    ...
    function getCurrentPlotSettingsFromUI_ACU() {
      ...
      return {
        ...
        contextTurnCount:
          parseInt($popupInstance_ACU.find(`#${SCRIPT_ID_PREFIX_ACU}-plot-context-turn-count`).val(), 10) || 3,
        loopSettings: {
          ...
          loopTotalDuration:
            parseInt($popupInstance_ACU.find(`#${SCRIPT_ID_PREFIX_ACU}-plot-loop-total-duration`).val(), 10) || 0,
  ...
  async function getWorldBooks_ACU() {
    if (
      TavernHelper_API_ACU &&
      typeof TavernHelper_API_ACU.getLorebooks === 'function' &&
      typeof TavernHelper_API_ACU.getLorebookEntries === 'function'
    ) {
  ...
  async function getCombinedWorldbookContent_ACU(initialScanTextOverride = '') {
    ...
    try {
      ...
      allEntries = allEntries.filter(
        entry => !entry.comment || !prefixesToExclude_ACU.some(prefix => entry.comment.startsWith(prefix)),
      );
      ...
      const baseScanText =
        typeof initialScanTextOverride === 'string' && initialScanTextOverride.trim()
      ...
      logDebug_ACU(
        `Combined worldbook content generated, length: ${combinedContent.length}. ${triggeredEntries.size} entries triggered.`,
      );
    ...
    tableIndexes.forEach((sheetKey, tableIndex) => {
      ...
      if (!targetSheetKeys) {
        ...
        const isStandardMode =
          updateMode === 'standard' || updateMode === 'auto_standard' || updateMode === 'manual_standard';
        const isSummaryMode =
          updateMode === 'summary' || updateMode === 'auto_summary_silent' || updateMode === 'manual_summary';
  ...
  async function callCustomOpenAI_ACU(dynamicContent) {
    ...
    if (effectiveApiMode === 'tavern') {
      ...
      try {
        ...
        const targetProfile = SillyTavern_API_ACU.extensionSettings?.connectionManager?.profiles.find(
          p => p.id === profileId,
        );
  ...
  function parseAndApplyTableEdits_ACU(aiResponse, updateMode = 'standard') {
    ...
    if (!extracted || !extracted.inner) {
      logWarn_ACU(
        'No recognizable table edit block found (missing <tableEdit> boundary and/or incomplete <!-- --> wrapper).',
      );
    ...
    originalLines.forEach(line => {
      ...
      if (
        (lineContent.startsWith('insertRow') ||
    ...
    commandLines.forEach(rawLine => {
      ...
      if (processedLine.match(/\[\d+:.*?\]-\s*(Update|Insert|Delete):/)) {
        logWarn_ACU(
          `Detected unstructured AI response format: "${rawLine}". Skipping this line as it is not a valid function call.`,
        );
        return;
      }
      ...
      try {
        ...
        switch (command) {
          case 'insertRow': {
            ...
            // [逻辑优化] 使用更明确的模式匹配
            const isUnifiedMode =
              updateMode === 'full' || updateMode === 'manual_unified' || updateMode === 'auto_unified';
            const isStandardMode =
              updateMode === 'standard' || updateMode === 'auto_standard' || updateMode === 'manual_standard';
            const isSummaryMode =
              updateMode === 'summary' ||
              updateMode === 'auto_summary' ||
              updateMode === 'auto_summary_silent' ||
              updateMode === 'manual_summary';
            ...
            if (isUnifiedMode) {
              ...
              if (isManualMode) {
                logDebug_ACU(
                  `[屏蔽] 标准表更新模式(手动)：忽略总结表/总体大纲的insertRow操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
                );
                break;
              }
              // 自动模式下不再屏蔽
              ...
              if (isManualMode) {
                logDebug_ACU(
                  `[屏蔽] 总结表更新模式(手动)：忽略标准表的insertRow操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
                );
                break;
              }
              // 自动模式下不再屏蔽
          ...
          case 'deleteRow': {
            ...
            if (isSummaryTable) {
              logDebug_ACU(
                `[屏蔽] 总结表/总体大纲忽略 deleteRow 操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
              );
              break;
            }
            ...
            if (isUnifiedMode) {
              ...
              if (isManualMode) {
                logDebug_ACU(
                  `[屏蔽] 标准表更新模式(手动)：忽略总结表/总体大纲的deleteRow操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
                );
                break;
              }
              // 自动模式下不再屏蔽
              ...
              if (isManualMode) {
                logDebug_ACU(
                  `[屏蔽] 总结表更新模式(手动)：忽略标准表的deleteRow操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
                );
                break;
              }
              // 自动模式下不再屏蔽
          ...
          case 'updateRow': {
            ...
            if (isSummaryTable) {
              logDebug_ACU(
                `[屏蔽] 总结表/总体大纲忽略 updateRow 操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
              );
              break;
            }
            ...
            if (isUnifiedMode) {
              ...
              if (isManualMode) {
                logDebug_ACU(
                  `[屏蔽] 标准表更新模式(手动)：忽略总结表/总体大纲的updateRow操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
                );
                break;
              }
              // 自动模式下不再屏蔽
              ...
              if (isManualMode) {
                logDebug_ACU(
                  `[屏蔽] 总结表更新模式(手动)：忽略标准表的updateRow操作 (tableIndex: ${tableIndex}, tableName: ${table.name})`,
                );
                break;
              }
              // 自动模式下不再屏蔽
            ...
            if (table && table.content && table.content.length > rowIndex + 1 && typeof data === 'object') {
              ...
              logDebug_ACU(
                `Applied updateRow to table ${tableIndex} (${table.name}) at index ${rowIndex} with data:`,
                data,
              );
              appliedEdits++;
              editCountsByTable[table.name] = (editCountsByTable[table.name] || 0) + 1;
    ...
    Object.keys(editCountsByTable).forEach(tableName => {
      if (editCountsByTable[tableName] > 0) {
        const sheetKey = Object.keys(currentJsonTableData_ACU).find(
          k => currentJsonTableData_ACU[k].name === tableName,
        );
  ...
  async function processUpdates_ACU(indicesToUpdate, mode = 'auto', options = {}) {
    ...
    logDebug_ACU(
      `[${mode}] Processing ${indicesToUpdate.length} updates in ${batches.length} batches of size ${batchSize} (${
        isSummaryMode ? '总结表模式' : '标准表模式'
      }). Target Sheets: ${targetSheetKeys ? targetSheetKeys.length : 'All'}`,
    );
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      logDebug_ACU(
        `[Batch ${batchNumber}] Loaded ${foundCount}/${totalCount} tables from history before index ${firstMessageIndexOfBatch}. Missing tables will use template structure (header-only).`,
      );
      ...
      if (sliceStartIndex > 0 && chatHistory[sliceStartIndex - 1]?.is_user) {
        sliceStartIndex--;
        logDebug_ACU(
          `[Batch ${batchNumber}] Adjusted slice start to ${sliceStartIndex} to include preceding user message.`,
        );
      }
      ...
      if (
        !isManualMode &&
        (mode === 'auto' || mode === 'auto_unified' || mode === 'auto_standard' || mode === 'auto_summary_silent') &&
        lastAiMessageLength < minReplyLength
      ) {
        logDebug_ACU(
          `[Auto] Batch ${batchNumber}/${totalBatches} skipped: Last AI reply length (${lastAiMessageLength}) is below threshold (${minReplyLength}).`,
        );
        // [新增] 静默模式下不显示跳过提示
        if (!isSilentMode) {
      ...
      const toastMessage = isSilentMode
        ? ''
        : `正在处理 ${isManualMode ? '手动' : '自动'} 更新 (${batchNumber}/${totalBatches})...`;
      // [修复] 传递 targetSheetKeys 到 proceedWithCardUpdate_ACU
      const success = await proceedWithCardUpdate_ACU(
        messagesForContext,
        toastMessage,
  ...
  async function checkAndTriggerAutoMergeSummary_ACU() {
    ...
    const summaryCount = summaryKey
      ? (currentJsonTableData_ACU[summaryKey].content || [])
          .slice(1)
          .filter(row => !row || row[row.length - 1] !== 'auto_merged').length
      : 0;

    const outlineCount = outlineKey
      ? (currentJsonTableData_ACU[outlineKey].content || [])
          .slice(1)
          .filter(row => !row || row[row.length - 1] !== 'auto_merged').length
      : 0;
    ...
    if (summaryCount >= triggerThreshold && outlineCount >= triggerThreshold) {
      ...
      if (mergeCount > 0) {
        logDebug_ACU(
          `触发自动合并总结: 总结表${summaryCount}条, 大纲表${outlineCount}条, 保留${reserve}条, 合并${mergeCount}条`,
        );
  ...
  async function performAutoMergeSummary_ACU(options) {
    ...
    let allSummaryRows = summaryKey
      ? (currentJsonTableData_ACU[summaryKey].content || [])
          .slice(1)
          .filter(row => !row || row[row.length - 1] !== 'auto_merged')
      : [];
    let allOutlineRows = outlineKey
      ? (currentJsonTableData_ACU[outlineKey].content || [])
          .slice(1)
          .filter(row => !row || row[row.length - 1] !== 'auto_merged')
      : [];
    ...
    try {
      ...
      for (let i = 0; i < totalBatches; i++) {
        ...
        const formatRows = (rows, globalStartIndex) =>
          rows.map((r, idx) => `[${globalStartIndex + idx}] ${r.slice(1).join(', ')}`).join('\n');
        const textA =
          batchSummaryRows.length > 0
            ? formatRows(batchSummaryRows, startIndex + 1 + startIdx)
            : '(本批次无新增总结数据)';
        const textB =
          batchOutlineRows.length > 0
        ...
        const formatTableStructure = (tableName, currentRows, originalTableObj, tableIndex) => {
          ...
          const headers = originalTableObj.content[0]
            ? originalTableObj.content[0]
                .slice(1)
                .map((h, i) => `[${i}:${h}]`)
                .join(', ')
            : 'No Headers';
        ...
        const getExistingAutoMergedRows = (tableKey, tableObj, count = 1) => {
          ...
          const withAm = autoMergedRows.map(r => ({ row: r, am: parseAmNumber(r) })).filter(x => Number.isFinite(x.am));
          ...
          const autoMergedOrder =
            settings_ACU.autoMergedOrder && settings_ACU.autoMergedOrder[tableKey]
              ? settings_ACU.autoMergedOrder[tableKey]
              : [];
        ...
        const existingSummaryAutoMerged = summaryTableObj
          ? getExistingAutoMergedRows(summaryKey, summaryTableObj, 1)
          : [];
        const existingOutlineAutoMerged = outlineTableObj
          ? getExistingAutoMergedRows(outlineKey, outlineTableObj, 1)
          : [];
        ...
        if (summaryTableObj)
          textBase += formatTableStructure(summaryTableObj.name, summaryBaseData, summaryTableObj, 0);
        if (outlineTableObj)
          textBase += formatTableStructure(outlineTableObj.name, outlineBaseData, outlineTableObj, 1);

        let currentPrompt = promptTemplate
          .replace('$TARGET_COUNT', targetCount)
          .replace('$A', textA)
          .replace('$B', textB)
          .replace('$BASE_DATA', textBase);
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            const messagesToUse = JSON.parse(
              JSON.stringify(settings_ACU.charCardPrompt || [DEFAULT_CHAR_CARD_PROMPT_ACU]),
            );
            ...
            if (settings_ACU.apiMode === 'tavern') {
              const result = await SillyTavern_API_ACU.ConnectionManagerRequestService.sendRequest(
              ...
              if (settings_ACU.apiConfig.useMainApi) {
                aiResponseText = await TavernHelper_API_ACU.generateRaw({
                  ordered_prompts: finalMessages,
                  should_stream: false,
                });
              } else {
                const res = await fetch(`/api/backends/chat-completions/generate`, {
                  ...
                  body: JSON.stringify({
                    ...
                    max_tokens: settings_ACU.apiConfig.max_tokens || 4096,
                    stream: false,
                    chat_completion_source: 'custom',
                    reverse_proxy: settings_ACU.apiConfig.url,
                    custom_url: settings_ACU.apiConfig.url,
                    custom_include_headers: settings_ACU.apiConfig.apiKey
                      ? `Authorization: Bearer ${settings_ACU.apiConfig.apiKey}`
                      : '',
                  }),
                });
        ...
        if (!isImportMode) {
          ...
          if (keysToPersist.length > 0 || isFirstTimeInit) {
            ...
            if (isFirstTimeInit) {
              ...
              if (fullTemplate) {
                allSheetKeys.forEach(sheetKey => {
                  ...
                  if (!keysToPersist.includes(sheetKey) && fullTemplate[sheetKey]) {
                    ...
                    logDebug_ACU(
                      `[Init] Table ${sheetKey} not modified by AI, using template data (may include seed rows).`,
                    );
                  }
                });
              }
              ...
              logDebug_ACU(
                '[Init] First time initialization detected. Saving complete template structure with all tables.',
              );
            }
  ...
  async function handleManualMergeSummary_ACU() {
    ...
    if (allSummaryRows.length === 0 && allOutlineRows.length === 0) {
      showToastr_ACU(
        'info',
        `指定范围内没有总结或大纲数据需要合并。范围: 第${startIndex + 1}条 到 第${actualEndIndex}条`,
      );
      return;
    }
    ...
    if (
      !confirm(
        `即将开始合并总结。\n\n源数据范围: 第${
          startIndex + 1
        }条 到 第${actualEndIndex}条 (${selectedRange} 条数据)\n处理数据: ${allSummaryRows.length} 条总结 + ${
          allOutlineRows.length
    ...
    let progressToast = showToastr_ACU('info', `<div>正在合并总结与大纲...${stopButtonHtml}</div>`, {
      ...
      tapToDismiss: false,
      onShown: function () {
        jQuery_API_ACU('#acu-merge-stop-btn')
          .off('click.acu_stop')
          .on('click.acu_stop', function (e) {
            e.stopPropagation();
            e.preventDefault();
            wasStoppedByUser_ACU = true;
    ...
    try {
      ...
      for (let i = 0; i < totalBatches; i++) {
        ...
        const formatRows = (rows, displayStartIndex) =>
          rows.map((r, idx) => `[${displayStartIndex + idx}] ${r.slice(1).join(', ')}`).join('\n');
        const textA =
          batchSummaryRows.length > 0
            ? formatRows(batchSummaryRows, startIndex + 1 + startIdx)
            : '(本批次无新增总结数据)';
        ...
        const summaryBaseData =
          i === 0 ? pickLastRowsBeforeIndex_ACU(fullSummaryRows, startIndex, 2) : accumulatedSummary.slice();
        const outlineBaseData =
          i === 0 ? pickLastRowsBeforeIndex_ACU(fullOutlineRows, startIndex, 2) : accumulatedOutline.slice();
    ...
    input.onchange = e => {
      ...
      reader.onload = async readerEvent => {
        ...
        try {
          ...
          // [新增] 导入所有合并设置 (如果存在)
          if (
            typeof combinedData.mergeSummaryPrompt !== 'undefined' ||
  ...
  async function deleteLocalDataInChat_ACU(mode = 'current', startFloor = null, endFloor = null) {
    ...
    // 计算AI消息索引列表（只计算AI楼层）
    const aiMessageIndices = chat.map((msg, index) => (!msg.is_user ? index : -1)).filter(index => index !== -1);
    ...
    if (deletedCount > 0) {
      ...
      showToastr_ACU(
        'success',
        `已成功删除 ${deletedCount} 条消息中的本地数据 (${mode === 'all' ? '所有数据' : '当前标识'})。`,
      );
    } else {
  ...
  async function resetAllToDefaults_ACU() {
    if (
      !confirm('确定要同时恢复【默认AI指令预设】和【默认表格模板】吗？\n\n这将覆盖您当前的自定义设置。此操作不可撤销。')
    ) {
      return;
    }
  ...
  async function overrideLatestLayerWithTemplate_ACU() {
    if (
      !confirm(
        '⚠️ 警告：此操作将使用当前通用模板覆盖聊天记录中最新一层的所有表格数据！\n\n' +
    ...
    if (modified) {
      ...
      showToastr_ACU(
        'success',
        `已使用通用模板覆盖最新层的${Object.keys(templateData).filter(k => k.startsWith('sheet_')).length}个表格数据。`,
      );
    } else {
  ...
  async function resetTableTemplate_ACU() {
    try {
      ...
      let obj = null;
      try {
        obj = JSON.parse(DEFAULT_TABLE_TEMPLATE_ACU);
      } catch (e) {}
      ...
      reader.onload = async readerEvent => {
        ...
        try {
          ...
          if (!jsonData.mate || !jsonData.mate.type || jsonData.mate.type !== 'chatSheets') {
            throw new Error(
              '缺少 "mate" 对象或 "type" 属性不正确。模板必须包含 `"mate": {"type": "chatSheets", ...}`。',
            );
          }
      ...
      const $item = jQuery_API_ACU(`
              <div class="acu-table-nav-item ${isActive ? 'active' : ''}" data-key="${key}">
                  <div class="acu-table-nav-content">
                      ...
                      <span style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1;">${escapeHtml_ACU(
                        sheet.name,
                      )}</span>
                  </div>
                  <div class="acu-table-nav-actions">
                      <button class="acu-table-order-btn acu-move-up-btn" data-key="${key}" title="上移" ${
        isFirst ? 'disabled' : ''
      }>
                          <i class="fa-solid fa-chevron-up"></i>
                      </button>
                      <button class="acu-table-order-btn acu-move-down-btn" data-key="${key}" title="下移" ${
        isLast ? 'disabled' : ''
      }>
      ...
      headers.forEach((header, cIdx) => {
        ...
        html += `
                  <div class="acu-field-row">
                      ...
                      <div class="acu-field-value" contenteditable="true" data-row="${rIdx}" data-col="${cIdx}">${escapeHtml_ACU(
          String(val),
        )}</div>
                  </div>
              `;
  ...
  function renderVisualizerConfigMode_ACU($container, sheet) {
          ...
          <div class="acu-config-panel">
              ...
              <div class="acu-config-section">
                  ...
                  <div class="acu-form-group">
                      ...
                      <input type="number" class="acu-form-input" id="cfg-depth" value="${
                        updateConfig.contextDepth || 0
                      }">
                  </div>
                  <div class="acu-form-group">
                      <label>更新频率 (Update Frequency): <span class="acu-hint">(每N层触发一次, 0 = 全局设置)</span></label>
                      <input type="number" class="acu-form-input" id="cfg-freq" value="${
                        updateConfig.updateFrequency || 0
                      }">
                  </div>
                  <div class="acu-form-group">
              ...
              <div class="acu-config-section">
                  ...
                  <div class="acu-form-group">
                      ...
                      <textarea class="acu-form-textarea" id="cfg-note">${escapeHtml_ACU(
                        sourceData.note || '',
                      )}</textarea>
                  </div>
                  <div class="acu-form-group">
                      <label>初始化触发 (Init):</label>
                      <textarea class="acu-form-textarea" id="cfg-init">${escapeHtml_ACU(
                        sourceData.initNode || '',
                      )}</textarea>
                  </div>
                  <div class="acu-form-group">
                      <label>新增触发 (Insert):</label>
                      <textarea class="acu-form-textarea" id="cfg-insert">${escapeHtml_ACU(
                        sourceData.insertNode || '',
                      )}</textarea>
                  </div>
                  <div class="acu-form-group">
                      <label>更新触发 (Update):</label>
                      <textarea class="acu-form-textarea" id="cfg-update">${escapeHtml_ACU(
                        sourceData.updateNode || '',
                      )}</textarea>
                  </div>
                  <div class="acu-form-group">
                      <label>删除触发 (Delete):</label>
                      <textarea class="acu-form-textarea" id="cfg-delete">${escapeHtml_ACU(
              ...
              <div class="acu-config-section">
                  ...
                  <div class="acu-form-group">
                      <label>
                          <input type="checkbox" id="cfg-inject" ${
                            config.injectIntoWorldbook !== false ? 'checked' : ''
                          }>
                  ...
                  <div style="border-top: 1px dashed #ddd; margin: 10px 0; padding-top: 10px;">
                      ...
                      <div id="cfg-export-options" style="display: ${
                        config.enabled ? 'block' : 'none'
                          ...
                          <div class="acu-form-group">
                              ...
                              <input type="text" class="acu-form-input" id="cfg-entry-name" value="${escapeHtml_ACU(
                                config.entryName || sheet.name || '',
                              )}" placeholder="例如: ${escapeHtml_ACU(sheet.name)}">
                          ...
                          <div class="acu-form-group">
                              ...
                              <select class="acu-form-input" id="cfg-entry-type">
                                  <option value="constant" ${
                                    !config.entryType || config.entryType === 'constant' ? 'selected' : ''
                                  }>常量条目 (Constant/Blue)</option>
                                  <option value="keyword" ${
                                    config.entryType === 'keyword' ? 'selected' : ''
                                  }>关键词条目 (Keyword/Green)</option>
                              </select>
                          </div>

                          <div class="acu-form-group">
                              <label>关键词 (Keywords):</label>
                              <input type="text" class="acu-form-input" id="cfg-keywords" value="${escapeHtml_ACU(
                                config.keywords || '',
                          ...
                          <div class="acu-form-group">
                              <label>
                                  <input type="checkbox" id="cfg-recursion" ${
                                    config.preventRecursion !== false ? 'checked' : ''
                                  }>
                          ...
                          <div class="acu-form-group">
                              ...
                              <textarea class="acu-form-textarea" id="cfg-template" placeholder="使用 $1 代表本表导出的蓝灯/绿灯条目列表，$1 上下的内容会分别生成独立的常量条目，插入到该表注入区块的最前与最后。">${escapeHtml_ACU(
                                config.injectionTemplate || '',
                              )}</textarea>
    ...
    // Inputs bindings
    jQuery_API_ACU('#cfg-name').on('input', function () {
      sheet.name = jQuery_API_ACU(this).val();
    });
    jQuery_API_ACU('#cfg-depth').on('input', function () {
      if (!sheet.updateConfig) sheet.updateConfig = {};
      sheet.updateConfig.contextDepth = parseInt(jQuery_API_ACU(this).val()) || 0;
    });
    jQuery_API_ACU('#cfg-freq').on('input', function () {
      if (!sheet.updateConfig) sheet.updateConfig = {};
      sheet.updateConfig.updateFrequency = parseInt(jQuery_API_ACU(this).val()) || 0;
    });
    jQuery_API_ACU('#cfg-batch').on('input', function () {
      if (!sheet.updateConfig) sheet.updateConfig = {};
      sheet.updateConfig.batchSize = parseInt(jQuery_API_ACU(this).val()) || 0;
    });
    jQuery_API_ACU('#cfg-skip').on('input', function () {
      if (!sheet.updateConfig) sheet.updateConfig = {};
      sheet.updateConfig.skipFloors = parseInt(jQuery_API_ACU(this).val()) || 0;
    });

    jQuery_API_ACU('#cfg-note').on('input', function () {
      if (!sheet.sourceData) sheet.sourceData = {};
      sheet.sourceData.note = jQuery_API_ACU(this).val();
    });
    jQuery_API_ACU('#cfg-init').on('input', function () {
      if (!sheet.sourceData) sheet.sourceData = {};
      sheet.sourceData.initNode = jQuery_API_ACU(this).val();
    });
    jQuery_API_ACU('#cfg-insert').on('input', function () {
      if (!sheet.sourceData) sheet.sourceData = {};
      sheet.sourceData.insertNode = jQuery_API_ACU(this).val();
    });
    jQuery_API_ACU('#cfg-update').on('input', function () {
      if (!sheet.sourceData) sheet.sourceData = {};
      sheet.sourceData.updateNode = jQuery_API_ACU(this).val();
    });
    jQuery_API_ACU('#cfg-delete').on('input', function () {
      if (!sheet.sourceData) sheet.sourceData = {};
      sheet.sourceData.deleteNode = jQuery_API_ACU(this).val();
    });

    // Worldbook Config Bindings
    const ensureExportConfig = () => {
  ...
  async function saveVisualizerChanges_ACU(saveToTemplate = false) {
    ...
    const deletedKeysToPurge_ACU = Array.isArray(_acuVisState.deletedSheetKeys)
      ? [..._acuVisState.deletedSheetKeys]
      : [];
        ...
        Object.keys(currentJsonTableData_ACU).forEach(key => {
          ...
          if (!templateObj[key]) {
            ...
            if (JSON.stringify(templateTable.sourceData) !== JSON.stringify(currentTable.sourceData)) {
              templateTable.sourceData = currentTable.sourceData
                ? JSON.parse(JSON.stringify(currentTable.sourceData))
                : {};
              hasChanges = true;
            }
            ...
            if (JSON.stringify(templateTable.updateConfig) !== JSON.stringify(currentTable.updateConfig)) {
              templateTable.updateConfig = currentTable.updateConfig
                ? JSON.parse(JSON.stringify(currentTable.updateConfig))
                : {};
              hasChanges = true;
            }
            ...
            if (JSON.stringify(templateTable.exportConfig) !== JSON.stringify(currentTable.exportConfig)) {
              templateTable.exportConfig = currentTable.exportConfig
                ? JSON.parse(JSON.stringify(currentTable.exportConfig))
                : {};
              hasChanges = true;
            }
      ...
      if (Object.keys(bucketByIndex).length === 0) {
        ...
        if (typeof purgeSheetKeysFromChatHistoryHard_ACU === 'function' && deletedKeysToPurge_ACU.length > 0) {
          try {
            ...
            if (r?.changed) {
              logDebug_ACU(
                `[VisualizerDelete] Hard-purged ${deletedKeysToPurge_ACU.length} keys from ${r.changedCount} AI messages.`,
              );
            }
            _acuVisState.deletedSheetKeys = [];
          } catch (e) {
